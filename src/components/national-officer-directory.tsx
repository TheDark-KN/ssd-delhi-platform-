...
export function NationalOfficerDirectory() {
  ...
  const states = useMemo(() => ["All states", ...Array.from(new Set(nationalOfficersEn.map((officer) => officer.state)))], []);
  const filteredOfficers = useMemo(() => {
    const term = search.trim().toLowerCase();
    return nationalOfficersEn.filter((officer) => {
      const matchesSearch = !term || `${officer.name} ${officer.designation} ${officer.state}`.toLowerCase().includes(term);
      return matchesSearch && (state === "All states" || officer.state === state);
    });
  }, [search, state]);
      ...
      <div className="container">
        <div className="mb-10 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="text-3xl font-black tracking-tight text-[#003285] md:text-5xl">Office Bearers</h2>
            <p className="mt-3 max-w-2xl text-base leading-relaxed text-slate-500 md:text-lg">Meet the national office bearers of Samta Sainik Dal. Each card includes a dedicated space for a portrait.</p>
          </div>
          <div className="rounded-2xl bg-[#003285] px-5 py-3 text-white shadow-lg shadow-[#003285]/15">
            <span className="block text-2xl font-black">{nationalOfficersEn.length}</span>
            <span className="text-xs font-bold uppercase tracking-widest text-blue-100">Office bearers</span>
          </div>
        </div>
        ...
        <div className="mb-10 grid gap-3 md:grid-cols-[1fr_220px]">
          <label className="relative block">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by name, role or state…" className="h-14 w-full rounded-2xl border border-slate-200 bg-slate-50 pl-12 pr-10 text-base outline-none transition focus:border-[#2A629A] focus:bg-white focus:ring-4 focus:ring-[#2A629A]/10" />
            {search && <button type="button" aria-label="Clear search" onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-1 text-slate-400 hover:bg-slate-200"><X className="h-4 w-4" /></button>}
          </label>
          <select aria-label="Filter by state" value={state} onChange={(event) => setState(event.target.value)} className="h-14 rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm font-bold text-[#003285] outline-none focus:border-[#2A629A] focus:ring-4 focus:ring-[#2A629A]/10">
