...
export default function EventPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      upcoming: "text-success bg-success/10",
          ...
          <div className="max-w-4xl mx-auto space-y-8">
            <div className="flex items-center gap-3 flex-wrap">
              <Badge className={cn("px-4 py-1 text-xs font-black uppercase tracking-widest rounded-full border-none",
                event.status === 'upcoming' ? "bg-success text-white shadow-lg shadow-success/20" :
                  event.status === 'ongoing' ? "bg-muted0 text-white shadow-lg shadow-primary/20" :
          ...
          <div className="grid gap-12 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2 space-y-12">
              <Card className="border-none shadow-2xl shadow-slate-200/50 rounded-[40px] md:rounded-[64px] overflow-hidden bg-white p-8 md:p-16">
                <div className="mb-12">
                  <h2 className="text-3xl font-black text-[#4F46E5] mb-6 uppercase tracking-tight">About This Event</h2>
              ...
              <Card className="border-none shadow-xl shadow-slate-100/60 rounded-[40px] overflow-hidden bg-white p-10">
                <div className="flex items-center gap-6">
                  <div className="h-20 w-20 rounded-3xl bg-[#4F46E5]/10 flex items-center justify-center text-[#4F46E5]">
                    <Users className="h-10 w-10" />
                  </div>
                  <div>
                    <div className="text-xs font-black text-[#F59E0B] uppercase tracking-[0.2em] mb-1">Organizer</div>
                    <div className="text-2xl font-black text-[#4F46E5]">SSD Delhi Central Unit</div>
            ...
            <div className="space-y-8">
              <Card className="border-none shadow-xl shadow-slate-100/50 rounded-[40px] overflow-hidden bg-[#4F46E5] text-white p-10">
                <h3 className="text-xl font-black mb-8 uppercase tracking-widest text-[#F59E0B]">Logistics</h3>
                <div className="space-y-8">
                  <div className="flex items-start gap-4">
                    <div className="h-10 w-10 rounded-xl bg-white/10 flex items-center justify-center text-[#F59E0B]">
                  ...
                  {event.registrationDeadline && (
                    <div className="flex items-start gap-4">
                      <div className="h-10 w-10 rounded-xl bg-[#F59E0B]/20 flex items-center justify-center text-[#F59E0B]">
                        <Ticket className="h-5 w-5" />
                      </div>
                      <div>
                        <div className="text-[10px] font-black uppercase tracking-widest text-[#F59E0B]/80 mb-1">Register By</div>
                        <div className="font-bold text-lg leading-tight text-[#F59E0B]">
              ...
              {/* Registration Card */}
              <Card className="border-none shadow-2xl shadow-accent/50 rounded-[40px] overflow-hidden bg-white p-10 group">
                {event.status === "upcoming" ? (
                  <div className="space-y-6">
                    <div className="flex items-center gap-3 text-success pt-2">
                      <div className="h-3 w-3 rounded-full bg-success animate-pulse" />
                      <span className="font-black uppercase tracking-widest text-xs">Registration Open</span>
                    </div>
                    <Button className="w-full h-16 bg-[#F59E0B] hover:bg-[#D97706] text-white rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-accent/30 group-hover:scale-[1.02] transition-all" size="lg" onClick={handleRegister}>
                      Confirm Presence
                    </Button>
                  ...
                  <div className="py-4 text-center">
                    ...
                    <span className="font-black text-[#4F46E5] uppercase tracking-widest text-xs">
                      Event {event.status}
                    </span>
                  </div>
                )}
