        ...
        <div className="container relative z-10 px-4 md:px-6">
          <div className="max-w-4xl space-y-6">
            ...
            <h1 className="text-5xl md:text-7xl font-black text-white tracking-tight leading-tight">
              News & <span className="text-[#F59E0B]">Notices</span>
            </h1>
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            ...
            {urgentNews.length > 0 && (
              <div className="lg:col-span-3">
                <h2 className="text-3xl font-black text-[#4F46E5] mb-8 flex items-center gap-3">
                  <Megaphone className="h-8 w-8 text-[#F59E0B]" />
                  Urgent Announcements
                </h2>
                <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                  {urgentNews.map((item: any) => (
                    <Link key={item._id} href={`/news/${item.slug}`} className="group">
                      <Card className="h-full border-none shadow-xl shadow-accent/50 rounded-[40px] overflow-hidden bg-white group-hover:-translate-y-2 transition-all duration-300">
                        <div className="p-10 border-t-8 border-[#F59E0B]">
                          <Badge className="bg-[#F59E0B] text-white border-none font-bold uppercase tracking-tighter text-[10px] px-3 py-1 rounded-full mb-6">
                            URGENT NOTICE
                          </Badge>
                          <CardTitle className="text-2xl font-black text-[#4F46E5] group-hover:text-[#F59E0B] transition-colors leading-tight line-clamp-2 mb-4">
                            {item.headline}
                          </CardTitle>
            ...
            <div className="lg:col-span-3 mt-16">
              <h2 className="text-3xl font-black text-[#4F46E5] mb-8">Latest News</h2>
              {!news || news.length === 0 ? (
                  ...
                  {regularNews.map((item: any) => (
                    <Link key={item._id} href={`/news/${item.slug}`} className="group">
                      <Card className="h-full border-none shadow-xl shadow-slate-100/60 rounded-[40px] overflow-hidden group-hover:-translate-y-2 transition-all duration-300">
                        <div className="p-10">
                          <Badge className="bg-[#4F46E5]/10 text-[#4F46E5] border-none font-black uppercase tracking-tighter text-[10px] px-3 py-1 rounded-full mb-6">
                            {item.category}
                          </Badge>
                          <CardTitle className="text-2xl font-black text-[#4F46E5] group-hover:text-[#F59E0B] transition-colors leading-tight line-clamp-2 mb-4">
                            {item.headline}
