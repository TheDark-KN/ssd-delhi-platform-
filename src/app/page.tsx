  ...
  return (
    <div className="bg-background">
      <section className="relative isolate overflow-hidden bg-forest text-cream">
        ...
        <div className="container relative flex min-h-[680px] items-end px-6 pb-20 pt-32 md:items-center md:pb-24">
          <div className="max-w-3xl">
            <p className="mb-6 flex items-center gap-3 text-sm font-semibold uppercase tracking-[0.25em] text-gold"><Leaf className="size-4" /> Samta Sainik Dal Delhi</p>
            <h1 className="max-w-3xl font-serif text-5xl leading-[0.98] tracking-tight md:text-7xl lg:text-8xl">Knowledge is the first step toward liberation.</h1>
            <p className="mt-8 max-w-2xl text-lg leading-8 text-cream/80 md:text-xl">A living archive of the movement for liberty, equality, and fraternity — rooted in the vision of Dr. B. R. Ambedkar.</p>
            <div className="mt-10 flex flex-wrap gap-4">
              <Button asChild size="lg" className="rounded-full bg-gold px-7 text-forest hover:bg-gold/90"><Link href="/history">Explore our history <ArrowRight data-icon="inline-end" /></Link></Button>
              <Button asChild size="lg" variant="outline" className="rounded-full border-cream/40 bg-transparent px-7 text-cream hover:bg-cream/10 hover:text-cream"><Link href="/join">Join the movement</Link></Button>
            </div>
          </div>
        </div>
      ...
      <section className="border-b border-border bg-background py-12">
        <div className="container grid grid-cols-2 gap-8 px-6 md:grid-cols-4">
          {[{ icon: Heart, value: "100+", label: "years of service" }, { icon: Users, value: "10K+", label: "members across India" }, { icon: Library, value: "500+", label: "programs and events" }, { icon: MapPin, value: "Pan India", label: "a growing network" }].map(({ icon: Icon, value, label }) => (
            <div key={label} className="flex items-start gap-3"><Icon className="mt-1 size-5 text-saffron" /><div><p className="font-serif text-3xl text-forest md:text-4xl">{value}</p><p className="mt-1 text-sm leading-5 text-muted-foreground">{label}</p></div></div>
          ))}
        </div>
      </section>

      <section className="container grid gap-12 px-6 py-24 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
